...
export const WordArtStudio: React.FC<WordArtStudioProps> = ({
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data.words) && data.words.length > 0) {
          ...
          setTimeout(() => generateWordArt(newWordsList), 50);
          return;
        }
      }
    } catch (err) {
  ...
  const generateWordArt = (wordsOverride?: WordItem[]) => {
    ...
    const targetWordsList = wordsOverride || words;
    let processedWords = [...targetWordsList]
